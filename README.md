...
##### Use first six letters (including "0X") of "Public address of the key" from step1 (node1), and convert them to decimal (use the link below). Use this decimal value as your network chain ID. In Mycrupto app: while configuring "Set Up your Custom Network", chain ID should be the one you added while setting up step 2. 
* There, you could transfer funds from old wallet to new wallet. (Key is using local network (same network for multiple wallets transactions))
